    ...
    <div>
      <div className="relative">
        ...
        <RdxSelect.Root value={value} onValueChange={handleSelect}>
          <RdxSelect.Trigger
            className={cn(
              "w-full bg-white rounded-lg border border-gray-500 px-3 h-[52px] text-gray-800 pt-4 focus:border-gray-800 transition-all outline-none text-left relative flex items-center",
            ...
            <RdxSelect.Content className="z-[99] overflow-hidden bg-white rounded-2xl border border-gray-100 shadow-[0px_11px_20px_0px_rgba(0,0,0,0.10)]">
              ...
              <RdxSelect.Viewport className="p-2">
                {options?.map(option => (
                  <RdxSelect.Item
                    key={option.value}
                    className="p-2 text-gray-800 text-sm data-[state=checked]:font-bold data-[highlighted]:bg-gray-50 rounded-lg transition-colors"
                    value={option.value}
                  >
